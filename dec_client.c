...
int main(int argc, char* argv[]) {
    ...
    fgets(ciphertext, sizeof(ciphertext), CipherFIle);
    ...
    NoGoodCipher = NoGoodChecker(ciphertext, charset);
    NoGoodKey = NoGoodChecker(key, charset);
    if (NoGoodCipher || NoGoodKey) {
    ...
    sprintf(buffer, "dec,%s,%s,", plaintext, key);
    charsWritten = 0;
